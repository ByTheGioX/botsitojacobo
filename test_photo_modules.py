...
def _open_wa_chat(link):
    ...
    # Only bail out if we can POSITIVELY detect the QR screen.
    # Do NOT use _wa_is_connected() here — it returns False for valid
    # intermediate states like WhatsApp's "Continuar al chat" confirmation
    # dialog, which would prevent the dialog-click code below from running.
    try:
        _post_nav_src = wb.web_browser.page_source
        if 'data-testid="qrcode"' in _post_nav_src or 'data-ref=' in _post_nav_src:
            print('  [WARN] _open_wa_chat: QR detectado tras navegar (sesion caducada), saltando.')
            return None
    except Exception:
        pass
...
def _login_turitop():
    """Intenta loguearse en Turitop usando credenciales guardadas en
    data/turitop_credentials.json (estructura: {"email": "...", "password": "..."}).
    Asume que el browser YA esta en la pagina de login. Devuelve True si OK."""
    creds_fp = os.path.join(sys.path[0], 'data', 'turitop_credentials.json')
    if not os.path.exists(creds_fp):
        print('  [ALERTA] Turitop deslogueado y no hay credenciales guardadas.')
        print(f'           Crea el archivo: {creds_fp}')
        print('           Contenido: {"email": "tu_email", "password": "tu_password"}')
        return False
    try:
        with open(creds_fp, 'r', encoding='utf-8') as f:
            creds = json.load(f)
        email = (creds.get('email') or '').strip()
        password = creds.get('password') or ''
        if not email or not password:
            print(f'  [ALERTA] Credenciales Turitop incompletas en {creds_fp}')
            return False
    except Exception as e:
        print(f'  [ALERTA] No pude leer credenciales Turitop: {e}')
        return False

    print('  -> Intentando login automatico en Turitop...')
    try:
        email_input = None
        for _ in range(20):
            for sel in ("input[type='email']", "input[name='email']",
                ...
                try:
                    el = wb.web_browser.find_element(By.CSS_SELECTOR, sel)
                    if el.is_displayed():
                        email_input = el
                        break
                except Exception:
            ...
            time.sleep(0.5)
        if not email_input:
            print('  [WARN] No se encontro campo email en formulario de login.')
            return False
        email_input.clear()
        ...
        for sel in ("input[type='password']", "input[name='password']"):
            try:
                ...
                continue
        if not pwd_input:
            print('  [WARN] No se encontro campo password en formulario de login.')
            return False
        pwd_input.clear()
        pwd_input.send_keys(password)

        # "Mantenerme conectado" si existe (best-effort).
        try:
            for cb in wb.web_browser.find_elements(By.CSS_SELECTOR, "input[type='checkbox']"):
        ...
        submitted = False
        for sel in ("button[type='submit']", "input[type='submit']", "button[name='action']"):
            try:
                btn = wb.web_browser.find_element(By.CSS_SELECTOR, sel)
                if btn.is_displayed():
                    btn.click()
                    submitted = True
                    break
        ...
        for _ in range(20):
            time.sleep(0.5)
            if 'admin/login' not in wb.web_browser.current_url:
                print('  -> Login Turitop OK.')
                return True
        print('  [WARN] Login Turitop NO confirmado (sigue en /admin/login).')
        return False
    except Exception as e:
        print(f'  [ERROR] _login_turitop: {e}')
        return False
...
def scrape_turitop_for_date(target_day, target_month):
    ...
    try:
        ...
        if "admin/login/es/" in wb.web_browser.current_url:
            print("  [WARN] Turitop logged out! Intentando auto-login...")
            if _login_turitop():
                wb.get("https://app.turitop.com/admin/company/P271/bookings")
                time.sleep(5)
            if "admin/login/es/" in wb.web_browser.current_url:
                print("  [WARN] Auto-login fallo. Saltando este scrape.")
...
def _snapshot_outgoing_counts():
    """Cuenta los mensajes salientes con cada selector. Devuelve dict
    selector->count para comparar antes/después de un envío."""
    counts = {}
    for sel in _OUTGOING_MSG_SELECTORS:
        try:
            counts[sel] = len(wb.web_browser.find_elements(By.CSS_SELECTOR, sel))
        except Exception:
            counts[sel] = -1  # selector falló: se ignora en la comparación
    return counts
...
def match_and_send_photos(photo_entries):
    try:
        ...
        try:
            ...
            print(f'  pending_replies.json corrupted ({_je}), resetting.')
            json.dump([], open(pending_replies_fp, 'w'), indent=2)
            pending = []
...
"""
                ...
                if photo_confirmed:
                    ...
                    wa_already_pending = any(
                        p.get('wa_link', '').strip().lower() == wa_link_normalized
                        for p in pending
                    )
                    if not wa_already_pending:
                        pending.append({
        ...
        for entry in pending:
            try:
                ...
                if hours_elapsed >= 48 and not reminder_sent and not _client_text_reply:
                    ...
                    if attempts_so_far >= 10:
                        print(f'  Recordatorio con 10+ fallos → abandonando: {entry["booking_code"]}')
                        log_action(
                            booking_code=entry['booking_code'],
                            phone=entry['wa_link'].split('phone=')[-1],
                            status='ENVIADO',
                            response=f'recordatorio ABANDONADO ({attempts_so_far} fallos consecutivos)',
                            review='-'
                        )
                        continue  # Remove from pending
                    ...
                    fail_reason = ''
                    try:
                        # Reuse the chat_input from the reaction-check navigation above
                        # to avoid double-navigation (which triggers WhatsApp's intermediate
                        # "Continuar al chat" dialog and causes chat_input_not_found).
                        # Only re-navigate if the reaction block did not open the chat.
                        if _opened_chat_input is not None:
                            chat_input = _opened_chat_input
                        else:
...
"""
                        ...
                        if chat_input_2:
                            ...
                            for _sel in ["span[data-icon='send']", "span[data-icon='wds-ic-send-filled']", "button[aria-label='Enviar']", "div[aria-label='Enviar']"]:
                                try:
                                    _sb = wb.web_browser.find_element(By.CSS_SELECTOR, _sel)
                                    wb.web_browser.execute_script("arguments[0].click();", _sb)
                                    _rev_sent = True
                                    break
                                except Exception:
                                    continue
                            ...
                            if _rev_confirmed:
                                ...
                                print('  -> [ERROR] Review text send not confirmed, keeping in pending.')
                                updated_pending.append(entry)
                                continue
                        else:
                            print('  -> [ERROR] Could not find chat input for review text.')
                    ...
                    try:
                        ...
                        if _neg_confirmed:
                            ...
                            print(f'  -> [ERROR] Negative empathy send not confirmed, keeping in pending.')
                            updated_pending.append(entry)
                            continue
                    except Exception as ex:
    ...
    try:
        ...
        if "admin/login/es/" in wb.web_browser.current_url:
            ...
            if _login_turitop():
                ...
                time.sleep(5)
            if "admin/login/es/" in wb.web_browser.current_url:
                print("  [WARN] Auto-login fallo. Cannot dequeue emails.")
                wb.web_browser.close()
                wb.web_browser.switch_to.window(wb.web_browser.window_handles[0])
                return
    ...
    try:
        ...
        if os.path.exists(pending_replies_fp):
            try:
                ...
                print(f'  pending_replies.json corrupted en cleanup ({_je}), resetting.')
                json.dump([], open(pending_replies_fp, 'w'), indent=2)
                pending = []
            updated = []
            for entry in pending:
        ...
        # Trim photo_sent_messages.txt to last 2000 lines to prevent unbounded growth
        if os.path.exists(photo_sent_messages_fp):
            try:
                with open(photo_sent_messages_fp, 'r') as _f:
                    _lines = _f.readlines()
                if len(_lines) > 2000:
                    with open(photo_sent_messages_fp, 'w') as _f:
                        _f.writelines(_lines[-2000:])
                    print(f'  Trimmed photo_sent_messages.txt from {len(_lines)} to 2000 lines.')
            except Exception as _trim_err:
                print(f'  [WARN] Could not trim photo_sent_messages.txt: {_trim_err}')
...
# ============================================================
# SAFETY NET: ensure Chrome closes no matter how Python exits.
# Fires on: normal exit, sys.exit(), uncaught exceptions, SIGINT/SIGTERM.
# Does NOT fire on os._exit() or taskkill /f (no process can handle that).
# ============================================================
_browser_closed = False

def _safe_close_browser(reason=''):
    ...
    try:
        if reason:
            print(f"\n[cleanup] Cerrando navegador ({reason})...")
        else:
            print("\n[cleanup] Cerrando navegador...")
        wb.web_browser.quit()
        print("[cleanup] Navegador cerrado.")
    except Exception as _e_quit:
        print(f"[cleanup] [WARN] quit() falló: {_e_quit}")
    # Belt-and-suspenders: orphan chromedriver.exe / chrome.exe can keep the
    # user-data-dir locked (SingletonLock), blocking the next cycle.
    for _proc in ('chromedriver.exe',):
        try:
            subprocess.run(
                ['taskkill', '/F', '/IM', _proc],
...
atexit.register(_safe_close_browser, 'atexit')

def _on_signal(signum, _frame):
    print(f"\n[cleanup] Señal recibida ({signum}). Cerrando ordenadamente...")
    _safe_close_browser(f'signal {signum}')
    sys.exit(128 + signum)
...
try:
    ...
    print(f"[cleanup] [WARN] no se pudieron registrar señales: {_e_sig}")


# ============================================================
# CONTINUOUS LOOP — Browser stays open across cycles.
# This is the key difference from the previous single-shot model:
# WhatsApp Web session stays warm, so the bot doesn't get unlinked.
# ============================================================
_main_exit_code = 0
_cycle_num = 0
_CYCLE_SLEEP = 900   # 15 min between cycles when running normally
_OFFHOURS_SLEEP = 300  # 5 min check interval when outside operating hours
_WA_TIMEOUT = 60       # max seconds to wait for WhatsApp connection
...
while True:
    ...
    if _hours_enabled and (_cur_t < datetime.time(10, 0) or _cur_t >= datetime.time(20, 30)):
        ...
        try:
            time.sleep(_OFFHOURS_SLEEP)
        except KeyboardInterrupt:
            print("\n[INTERRUPT] Ctrl+C recibido — saliendo.")
            _main_exit_code = 130
            break
        continue
    ...
    _cycle_num += 1
    print("\n" + "=" * 60)
    print(f"  CICLO #{_cycle_num} — {_cycle_start_ts.strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 60)

    # --- Tab cleanup: keep only the main window ---
    try:
        handles = wb.web_browser.window_handles
        if len(handles) > 1:
            for h in handles[1:]:
                try:
                    wb.web_browser.switch_to.window(h)
                    wb.web_browser.close()
                except Exception:
                    pass
            wb.web_browser.switch_to.window(wb.web_browser.window_handles[0])
    ...
    # --- Navigate to / refresh WhatsApp Web (do NOT close browser) ---
    try:
        try:
            _cur_url = wb.web_browser.current_url or ''
        except Exception:
            _cur_url = ''
        if 'whatsapp.com' in _cur_url:
            print("Refrescando WhatsApp Web...")
        ...
        print(f"[WARN] Error navegando a WhatsApp Web ({_e_nav}). Reabriendo navegador...")
        try:
            wb.web_browser.quit()
        except Exception:
            pass
        _browser_closed = False  # allow atexit to close the new instance
        try:
            wb = Browser()
            wb.get("https://web.whatsapp.com")
            time.sleep(2)
            ...
            print(f"[ERROR] No se pudo reabrir el navegador: {_e_re}")
            try:
                time.sleep(60)
            except KeyboardInterrupt:
                _main_exit_code = 130
    ...
    # --- Wait for WhatsApp connection (max 60s) ---
    print(f"Esperando conexion de WhatsApp (max {_WA_TIMEOUT}s)...")
    _wa_connected = False
    _wa_start = time.time()
    _qr_shown = False
    ...
    if not _wa_connected:
        print("\n[WARN] WhatsApp no se conecto en 1 minuto. Saltando ciclo, reintento en 15 min.")
        # Persistent flag: stale wa_disconnect.flag means the bot is stuck on QR.
        try:
            flag_fp = os.path.join(sys.path[0], 'data', 'wa_disconnect.flag')
            with open(flag_fp, 'w', encoding='utf-8') as _flag:
                _flag.write(datetime.datetime.now().isoformat())
        except Exception:
            pass
        try:
            write_execution_log('NEGATIVO', 'WhatsApp no conectado (QR visible) — ciclo abortado')
        except Exception:
            pass
        # IMPORTANT: do NOT close the browser, do NOT exit. Just wait and retry.
        try:
    ...
    # Connected — clear any stale disconnect flag.
    try:
        flag_fp = os.path.join(sys.path[0], 'data', 'wa_disconnect.flag')
        if os.path.exists(flag_fp):
            os.remove(flag_fp)
    except Exception:
        pass
    ...
    try:
        ...
        print("\n" + "=" * 60)
        print(f"  CICLO #{_cycle_num} COMPLETADO")
        print("=" * 60)

    except KeyboardInterrupt:
        print("\n[INTERRUPT] Ctrl+C recibido — saliendo del bot.")
        try:
            write_execution_log('NEGATIVO', 'Ctrl+C: ciclo abortado por usuario')
        except Exception:
            pass
        _main_exit_code = 130
        break
        ...
        # Module raised something its own try/except missed. Log it and
        # continue to the next cycle — don't kill the bot.
        print(f"\n[ERROR] Fallo no manejado en este ciclo: {_top_err}")
        try:
            traceback.print_exc()
        except Exception:
            pass
        try:
            write_execution_log('NEGATIVO', f'Fallo no manejado: {_top_err}')
        except Exception:
            pass

    # --- Sleep 15 min before next cycle (browser stays open) ---
    print(f"\n[{datetime.datetime.now().strftime('%H:%M:%S')}] "
          f"Esperando {_CYCLE_SLEEP//60} min hasta el proximo ciclo...")
    try:
        time.sleep(_CYCLE_SLEEP)
    except KeyboardInterrupt:
        print("\n[INTERRUPT] Ctrl+C recibido — saliendo.")
...
_safe_close_browser('normal exit' if _main_exit_code == 0 else f'exit {_main_exit_code}')
sys.exit(_main_exit_code)
