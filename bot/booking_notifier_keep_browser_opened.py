        ...
        if send_message:
            ...
            if not ("Enlace incorrecto. Cierra la venta y vuelve a intentarlo con un enlace diferente." in wb.web_browser.page_source) and not ("El número de teléfono compartido a través de la dirección URL es inválido" in wb.web_browser.page_source) and not("El número de teléfono compartido a través de la dirección URL no es válido" in wb.web_browser.page_source) and not("no está en WhatsApp" in wb.web_browser.page_source):
                ...
                if not ("El número de teléfono compartido a través de la dirección URL es inválido" in wb.web_browser.page_source) and not("El número de teléfono compartido a través de la dirección URL no es válido" in wb.web_browser.page_source):
                    time.sleep(3)
                    # Step 1: send text message first (separate from attachment)
                    wb.css_click("button[aria-label=Enviar],span[data-icon='wds-ic-send-filled'],span[data-icon='send']")
                    time.sleep(3)
                    # Step 2: open attachment panel
                    wb.css_click("span[data-icon='plus-rounded'],div[title='Adjuntar'],button[title='Adjuntar']")
                    time.sleep(3)
                    ...
                    time.sleep(5)
                    try:
                        msgs_before_photo = len(wb.web_browser.find_elements(By.CSS_SELECTOR, 'div.message-out'))
                    except:
                        msgs_before_photo = 0
                    ...
                    for _ci in range(10):
                        try:
                            if len(wb.web_browser.find_elements(By.CSS_SELECTOR, 'div.message-out')) > msgs_before_photo:
                ...
                print(f'send error: {_send_err}, line: {_send_err.__traceback__.tb_lineno}')
            finally:
                # Always record to prevent duplicate sends even if an exception occurred
...
"""
                    ...
                    # Count outgoing messages BEFORE clicking send to detect a NEW message
                    try:
                ...
                if send_success:
                    ...
                    # Add to pending replies only if this wa_link isn't already pending
                    wa_link_already_pending = any(
                        p.get('wa_link', '').strip().lower() == wa_link_normalized
                        for p in pending
                    )
                    if not wa_link_already_pending:
                        pending.append({
                        ...
                        print(f'  wa_link already in pending replies, skipping duplicate entry.')
                else:
...
def check_pending_replies():
    ...
    try:
        ...
        try:
            ...
            print(f'pending_replies.json corrupted ({_je}), resetting file.')
            json.dump([], open(pending_replies_fp, 'w'), indent=2)
            return
        if len(pending) == 0:
        ...
        for entry in pending:
            try:
                ...
                # Safety check: verify we haven't already sent a response after the client's reply.
                # Find the last message-in index, then check if any message-out exists after it.
                last_in_index = None
                for i in range(len(all_msgs) - 1, -1, -1):
                    if 'message-in' in (all_msgs[i].get_attribute('class') or ''):
                        last_in_index = i
                        break
                if last_in_index is not None and last_in_index < len(all_msgs) - 1:
                    # There is a message-out AFTER the last client reply — we already responded
                    print(f'already responded after client reply for: {entry["booking_code"]}, removing from pending.')
                    continue
                ...
                def _send_reply_msg(msg_text):
                    """Type msg_text and click send button. Returns True on success."""
                    try:
                        chat_input = wb.web_browser.find_element(
                            By.CSS_SELECTOR, "footer div[contenteditable='true']"
                        )
                        ...
                        chat_input.send_keys(Keys.CONTROL, 'v')
                        time.sleep(1)
                        # Use send button (more reliable than ENTER which may insert newline)
                        sent = wb.css_click(
                            "button[aria-label=Enviar],span[data-icon='wds-ic-send-filled'],"
                            "span[data-icon='send'],div[aria-label='Enviar']"
                        )
                        time.sleep(3)
                        return sent
                    except Exception as _ex:
                        print(f'  error in _send_reply_msg: {_ex}')
                        return False
                ...
                if classification == 'POSITIVO':
                    ...
                    if not _send_reply_msg(review_msg):
                        print('error sending positive review msg, keeping in pending.')
                        updated_pending.append(entry)
                        continue
                    ...
                    if not _send_reply_msg(neg_msg):
                        print('error sending negative empathy msg, keeping in pending.')
                        updated_pending.append(entry)
                        continue
...
def daily_cleanup():
    ...
    try:
        ...
        # Trim sent-message log files to last 2000 lines to prevent unbounded growth
        for _trim_fp in [sent_messages_fp, photo_sent_messages_fp]:
            try:
                if os.path.exists(_trim_fp):
                    with open(_trim_fp, 'r', encoding='utf-8', errors='ignore') as _f:
                        _lines = _f.readlines()
                    if len(_lines) > 2000:
                        with open(_trim_fp, 'w', encoding='utf-8') as _f:
                            _f.writelines(_lines[-2000:])
                        print(f'trimmed {_trim_fp} to last 2000 entries.')
            except Exception as _te:
                print(f'could not trim {_trim_fp}: {_te}')
...
def _safe_close_browser(reason=''):
    ...
    # Belt-and-suspenders: orphan chromedriver.exe can keep user-data-dir
    # locked (SingletonLock) and block the next cycle.
    try:
        subprocess.run(
